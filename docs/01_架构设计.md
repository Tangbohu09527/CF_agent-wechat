...
7. Gateway `wechat-worker` running/healthy，且可用 heartbeat 正常。
...
权限、Hermes 调用、Skills 与企业系统均是外部边界。本仓库只协调
`wechat-worker`，不启停 `dispatch-worker` 或 `delivery-worker`，不修改 Gateway、
PostgreSQL、Checkpoint 或其他仓库。
