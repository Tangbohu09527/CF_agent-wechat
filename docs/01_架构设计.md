...
- 生产设置为 `ENABLE_VNC=0`。
- forced-QR 目标基线的唯一启动入口为 `./scripts/start-qr-login.sh`，每次使用全新 runtime 和二维码。
- 本仓库只能在脚本运行后停止并复核 worker；Gateway boot/restart stop gate 必须在
  CFserver 实机验证。

VNC、noVNC、x11vnc、websockify、宿主 X11 挂载、宿主 XFCE 和 RDP 均不在
forced-QR 目标生产链路中。它们只可能出现在明确标为“历史实验、已废弃、非当前生产方案”
的旧记录里。
...
| Gateway | 通过 `cf-internal` 调用本服务 | 内部权限与编排不属于本项目 |
...
等待。2026-08-13 已信任设备的 `phone_confirm` 只属于历史基线；目标完整流程仍待
CFserver 实机验证。详细说明见 [微信登录管理](login-management.md)。

## 网络与调用关系

`cf-agent-wechat` 与外部 Gateway 调用方接入 `cf-internal`。截至 2026-08-13，
...
目标运行单元为 `/srv/storage/cf-agent-wechat/runtime/data` 和
...
实际存在的目录进入同一个归档；新旧布局并存时 fail-fast。
...
`status.sh` 返回 `0` 还要求配置、Token 和容器/worker 查询可用、容器运行、
forced-QR mounts 为 fresh；`Agent Server` 健康结果只展示，worker 为 stopped 时仍
可能返回 `0`。完成启动还要确认第七项 worker 为 running，以及非空 chats 和 messages
验证已通过。`logged_out`、进程不存在或消息 API 不可读都必须回到 forced-QR
生命周期；步骤见 [故障排查](troubleshooting.md)。
